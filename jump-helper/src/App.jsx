...
export default function App() {
  ...
  async function initApp() {
    try {
      await supabase.from('duty_history')
      ...
      const { data: wRows } = await supabase
        .from('duty_weights').select('*')
        .order('updated_at', { ascending: false }).limit(1)

      if (!wRows || wRows.length === 0) {
        const { data: wNew } = await supabase
          .from('duty_weights')
          .insert({ version: WEIGHTS_VERSION, ...INITIAL_WEIGHTS }).select()
  ...
  const saveEntry = useCallback(async (date, person, isHoliday) => {
    ...
    await Promise.all([
      ...
      supabase.from('duty_ml_log').insert({
        date, actual_person: person, is_holiday: isHoliday,
        predicted_rank: rank || 99, predicted_prob: actualPred?.prob ?? 0,
        top5, weights_before: { ...weights }, weights_after: newWeights, delta,
      }),
    ])
    if (weightsId) {
      await supabase.from('duty_weights')
        .update({ ...newWeights, updated_at: new Date().toISOString() }).eq('id', weightsId)
    } else {
      const { data: wNew } = await supabase
        .from('duty_weights').insert({ version: WEIGHTS_VERSION, ...newWeights }).select()
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100dvh', background:'var(--bg)' }}>
      ...
      <header style={{
        ...
        <span style={{ fontFamily:'var(--display)', fontWeight:800, fontSize:17, color:'var(--cyan)', letterSpacing:-0.3 }}>
          ⚡ JUMP
        </span>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <span className={`pill ${todayHol ? 'pill-amber' : 'pill-cyan'}`}>
            {todayHol ? '휴일' : '평일'}
          </span>
        </div>
      </header>
      ...
      <nav className="tab-nav">
        {TABS.map((t, i) => (
          <button key={i} className={`tab-item ${activeTab === i ? 'active' : ''}`}
