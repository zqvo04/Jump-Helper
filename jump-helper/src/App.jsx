...
export default function App() {
  ...
  const setNonROT=useCallback(async(id,isNonRot)=>{
    let newOrder
    if(!isNonRot){const rotMems=members.filter(m=>!(m.is_non_rot??NON_ROT_MEMBERS.includes(m.name))&&m.is_active&&!m.is_retired);newOrder=rotMems.length>0?Math.max(...rotMems.map(m=>m.display_order||0))+1:1}
  ...
  const today=getTodayStr(), todayHol=isHolidayDate(today)
  ...
  return (
    <div style={{ display:'flex', height:'100dvh', overflow:'hidden', background:'var(--bg)' }}>
      ...
      {/* Mobile backdrop */}
      {!isDesktop && sideOpen && (
        <div onClick={()=>setSideOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:98, backdropFilter:'blur(3px)' }}/>
      )}
      ...
      <aside style={{
        width: 220,
        flexShrink: 0,
        background: 'linear-gradient(180deg,#C62828 0%,#7B0000 100%)',
        display: 'flex',
        flexDirection: 'column',
        overflow: 'hidden',
        zIndex: 100,
        // DESKTOP: in-flow / MOBILE: fixed overlay
        position: isDesktop ? 'relative' : 'fixed',
        top:0, bottom:0, left:0,
        transform: isDesktop ? 'none' : (sideOpen ? 'translateX(0)' : 'translateX(-220px)'),
        transition: isDesktop ? 'none' : 'transform 0.25s cubic-bezier(.4,0,.2,1)',
        boxShadow: isDesktop ? '2px 0 16px rgba(0,0,0,0.1)' : '4px 0 24px rgba(0,0,0,0.2)',
      }}>

        {/* Logo */}
        <div style={{ padding:'20px 18px 14px', borderBottom:'1px solid rgba(255,255,255,0.12)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:40, height:40, borderRadius:12, background:'rgba(255,255,255,0.2)', border:'2px solid rgba(255,255,255,0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0, boxShadow:'0 2px 8px rgba(0,0,0,0.15)' }}>⚡</div>
            <div>
              <div style={{ fontWeight:900, fontSize:15, color:'#FFF', letterSpacing:-0.3 }}>Jump Helper</div>
              <div style={{ fontSize:9, color:'rgba(255,255,255,0.45)', marginTop:1, fontFamily:'var(--mono)' }}>Duty Oracle v5.1</div>
            </div>
          </div>
        </div>

        {/* Today box */}
        <div style={{ margin:'12px 12px 4px', padding:'10px 12px', borderRadius:10, background:'rgba(0,0,0,0.2)', border:'1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ fontSize:9, color:'rgba(255,255,255,0.4)', fontWeight:800, letterSpacing:0.08 }}>TODAY</div>
          <div style={{ fontWeight:800, fontSize:14, color:'#FFF', marginTop:3 }}>{formatDateKo(today)}</div>
          <div style={{ display:'flex', gap:6, marginTop:6, flexWrap:'wrap' }}>
            <span style={{ fontSize:11, padding:'2px 9px', borderRadius:99, background:todayHol?'rgba(255,167,38,0.22)':'rgba(76,175,80,0.22)', color:todayHol?'#FFD54F':'#A5D6A7', fontWeight:700 }}>
              {todayHol?'🌙 휴일':'☀️ 평일'}
            </span>
            {top1Pct!==null&&<span style={{ fontSize:11, padding:'2px 9px', borderRadius:99, background:'rgba(255,255,255,0.12)', color:'#FFF', fontWeight:700 }}>🎯 {top1Pct}%</span>}
          </div>
        </div>

        {/* Nav */}
        <nav style={{ flex:1, padding:'8px 0', overflowY:'auto' }}>
          <div style={{ padding:'6px 18px 4px', fontSize:9, fontWeight:800, color:'rgba(255,255,255,0.3)', letterSpacing:0.1 }}>MENU</div>
          {TABS.map((t,i)=>(
            <button key={i} onClick={()=>selectTab(i)} style={{
              ...
              fontWeight: activeTab===i ? 800 : 600, fontSize:13,
        ...
        <div style={{ padding:'12px', borderTop:'1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ fontSize:9, color:'rgba(255,255,255,0.3)', marginBottom:8, fontWeight:800, letterSpacing:0.08 }}>TEAM</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6 }}>
            {[['⭐',goodCnt,'좋은사람','rgba(76,175,80,0.2)','#A5D6A7'],['👥',members.filter(m=>m.is_active&&!m.is_retired).length,'전체','rgba(255,255,255,0.1)','#FFF'],['🍄',badCnt,'위험군','rgba(239,83,80,0.2)','#EF9A9A']].map(([icon,cnt,label,bg,color])=>(
              <div key={label} style={{ textAlign:'center', padding:'7px 4px', borderRadius:8, background:bg }}>
                <div style={{ fontSize:13 }}>{icon}</div>
                <div style={{ fontFamily:'var(--mono)', fontWeight:900, fontSize:16, color, lineHeight:1, marginTop:2 }}>{cnt}</div>
                <div style={{ fontSize:9, color, opacity:0.75, marginTop:2 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <header style={{
          ...
          padding:`0 ${isDesktop?28:16}px`, height:58, flexShrink:0,
          display:'flex', alignItems:'center', justifyContent:'space-between',
          boxShadow:'0 2px 8px rgba(26,31,54,0.05)',
        }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            {!isDesktop && (
              <button onClick={()=>setSideOpen(v=>!v)} style={{
                ...
                border:`1.5px solid ${sideOpen?'var(--side-dk)':'var(--border)'}`,
                cursor:'pointer', padding:0, transition:'all 0.15s',
              }}>
                {[0,1,2].map(i=>(
                  <div key={i} style={{ width:15, height:2, borderRadius:1, background:sideOpen?'#FFF':'var(--text2)', transition:'all 0.2s',
                    transform:sideOpen?(i===0?'rotate(45deg) translate(3px,3px)':i===2?'rotate(-45deg) translate(3px,-3px)':'scaleX(0)'):'none' }}/>
                ))}
              </button>
            )}
            <div>
              <div style={{ fontWeight:900, fontSize:isDesktop?20:16, color:'var(--text)', lineHeight:1 }}>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            ...
            <span style={{ padding:'5px 13px', borderRadius:99, fontSize:12, fontWeight:800,
              background:todayHol?'rgba(230,81,0,0.08)':'rgba(46,125,50,0.08)',
        ...
        <main className="scroll-y" style={{ flex:1, background:'var(--bg)' }}>
          <div style={{
            padding: isDesktop ? '28px 36px' : '14px',
            paddingBottom: 40,
            maxWidth: isDesktop ? 1320 : 'none',
...
function Err({msg}){return(<div style={{height:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'var(--bg)',gap:12,padding:24}}><span style={{fontSize:40}}>🍄</span><p style={{fontWeight:900,fontSize:16,color:'var(--red)'}}>연결 오류</p><p style={{color:'var(--text2)',fontSize:13,textAlign:'center',lineHeight:1.6,background:'var(--s1)',padding:'12px 16px',borderRadius:12}}>{msg}</p></div>)}
