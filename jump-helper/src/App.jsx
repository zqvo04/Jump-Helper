...
export default function App() {
  ...
  return (
    <div style={{display:'flex',height:'100dvh',overflow:'hidden',background:'var(--bg)'}}>
      ...
      {/* Mobile backdrop */}
      {sideOpen && (
        <div onClick={()=>setSideOpen(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:98,backdropFilter:'blur(2px)'}} className="hide-desktop"/>
      )}
      ...
      <aside className={`app-sidebar ${sideOpen?'':'closed'}`} style={{
        ...
        <div style={{margin:'10px 10px 4px',padding:'10px 12px',borderRadius:10,background:'rgba(0,0,0,0.18)'}}>
          <div style={{fontSize:10,color:'rgba(255,255,255,0.45)',fontWeight:700,letterSpacing:0.06}}>TODAY</div>
        ...
        <div style={{padding:'12px',borderTop:'1px solid rgba(255,255,255,0.1)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6}}>
            {[['⭐',goodCnt,'좋은사람','rgba(76,175,80,0.25)','#A5D6A7'],['👥',members.filter(m=>m.is_active&&!m.is_retired).length,'전체','rgba(255,255,255,0.1)','#FFF'],['🍄',badCnt,'위험군','rgba(239,83,80,0.25)','#EF9A9A']].map(([icon,cnt,label,bg,color])=>(
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
        ...
        {/* Header */}
        <header style={{background:'var(--s1)',borderBottom:'1.5px solid var(--border)',padding:'0 20px',height:56,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between',boxShadow:'0 2px 8px rgba(26,31,54,0.06)'}}>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            {/* Hamburger — mobile only */}
            <button className="hide-desktop" onClick={()=>setSideOpen(v=>!v)} style={{
              width:38,height:38,borderRadius:9,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:5,
              background:sideOpen?'var(--red)':'var(--s3)',border:`1.5px solid ${sideOpen?'#8B0000':'var(--border)'}`,
          ...
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            ...
            <span style={{padding:'5px 12px',borderRadius:99,fontSize:13,fontWeight:800,background:todayHol?'rgba(230,81,0,0.09)':'rgba(46,125,50,0.09)',color:todayHol?'var(--amber)':'var(--green)',border:`1.5px solid ${todayHol?'rgba(230,81,0,0.2)':'rgba(46,125,50,0.2)'}`}}>
              {todayHol?'휴일':'평일'}
            </span>
          </div>
        </header>
